...
public class ExcelExporterReport {
    ...
    private List<Attendance> listLogs;

    private AttendanceRepository logDetailRepository;
    private long id;
    ...
    public ExcelExporterReport(List<Attendance> listLogs, Long id, int month, DepartmentRepository departmentRepository,
                               UserRepository userRepository,
                               AttendanceRepository logDetailRepository) {
    ...
    public ExcelExporterReport(List<Attendance> listLogs, int month, DepartmentRepository departmentRepository,
                               UserRepository userRepository,
                               AttendanceRepository logDetailRepository) {
        this.listLogs = listLogs;
        workbook = new XSSFWorkbook();
...
//        List<User> listUsers = userRepository.findAllByDepartments_Id(id);
        List<UserAttendance> userLogDetails = new ArrayList<>();


        for (User user : listUsers) {
            UserAttendance userLogDetail = new UserAttendance();
            List<Attendance> list = new ArrayList<>();
            for (Attendance logDetail : listLogs) {
                if (logDetail.getUser() == user) {
                    list.add(logDetail);
                } else continue;
                userLogDetail.setCode(user.getUserCode());
                userLogDetail.setName(user.getFullName());
                userLogDetail.setAttendances(list);
            }
        ...
        for (User user : listUsers) {
            ...
            for (UserAttendance ul : userLogDetails) {
                if (user.getUserCode() == ul.getCode()) {
                    ...
                    for (Attendance logDetail : logDetails) {
                        ...
                        if (logDetail.getSigns() == null) {
                            ...
                            cell.setCellStyle(styleBodyCenter);
                            //Comment
                            if (logDetail.getEditReason() != null) {
//                                ClientAnchor clientAnchor = drawing.createAnchor(0, 0, 0, 0, 0, 2, 7, 7);
                                ...
                                RichTextString richTextString = creationHelper.createRichTextString(logDetail.getEditReason());
